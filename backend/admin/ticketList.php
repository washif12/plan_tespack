...
if(isset($_POST['id'])):
    $id = $_POST['id'];
    try{
        $check = "SELECT * FROM ticket_support WHERE id=:id";
        ...
        if($check_stmt->rowCount()):
            ...
            //$values = ['fields' => [$data["ref"],$data["model"],$data["contact"],$data["date"],$data["country"],$data["note"],$data["id"]]];
            $returnData = msg(1,201,'Data Received',$data);
        else:
