...
function validateUser($data)
{
    ...
    try {
        ...
        $res = $query->fetch();
        if($res['res'] == 0){
            $response['body']['error_msg'] = "Invalid username or password provided";
            $response['body']['status'] = 0;
         }
         if($res['res'] == 1){
            $query1 =  $conn->prepare("select user_name, user_id, status from tbm_user where user_name = ?  and password = md5(?) ");
            $query1->execute(array($data['user_name'], $data['password']));
            $query1->setFetchMode(PDO::FETCH_ASSOC);
            $ress = $query1->fetch();
            $response['body']['msg'] =  "ok";
            $response['body']['status'] = 1;
            $response['body']['user_details'] = $ress;
          }
    } catch (\PDOException $e) {
        exit($e->getMessage());
    }
...
function IDIsExist($data)
{
    ...
    try {
        $sql1 = "SELECT *, (select EXISTS (SELECT * FROM tbm_battery_info WHERE battery_id = ? ))as  res FROM tbm_battery_info WHERE battery_id = ?";
