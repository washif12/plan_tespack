...
function insertData($data)
{
    ...
    if (!array_key_exists("longitude", $data)) {
        return unprocessableEntityResponse('longitude is required');
    }
    if (!array_key_exists("speed_over_ground", $data)) {
        return unprocessableEntityResponse('speed_over_ground is required');
    }
    if (!array_key_exists("course_over_ground", $data)) {
        return unprocessableEntityResponse('course_over_ground is required');
    }
    $db_connection = new Database();
    $conn = $db_connection->dbConnection();
    ...
    if ($res_ssm_id['ssm_id'] != 1) {
        ...
        try {
            $sql = "INSERT INTO gms (latitude, longitude, ssm_id, speed_over_ground, course_over_ground, created_at, updated_at) 
            VALUES (:latitude, :longitude, :ssm_id, :speed_over_ground, :course_over_ground, NOW(), NOW())";
            $sth = $conn->prepare($sql);
            $status = $sth->execute(array(
                'latitude' => trim($data['latitude']),
                'longitude'  => trim($data['longitude']),
                'ssm_id' => trim(strtoupper($data['ssm_id'])),
                'speed_over_ground' => trim($data['speed_over_ground']),
                'course_over_ground' => trim($data['course_over_ground']),
