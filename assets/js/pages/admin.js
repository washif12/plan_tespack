$(document).ready(function() {
    ...
    $("#inviteBtn").click(function(event) {
        ...
        $.ajax({
            ...
            if (data.success == 0) {
                ...
                $("#errMsg").html('<i class="fa fa-times-circle"></i>' + data.message).show().delay(3000).hide("slow");
                ...
                $("#msgSuccess").html('<i class="fa fa-check-circle"></i>' + data.message).show().delay(2000).hide("slow", insertModalReset);
            }
        });
    });
    /* Add user Manually */
    $("#addBtn").click(function(event) {
        event.preventDefault()
        var formDataManual = {
            fname: $("#fnameManual").val(),
            lname: $("#lnameManual").val(),
            email: $("#emailManual").val(),
            role: $("#roleManual").val(),
            phone: $("#phoneManual").val(),
            country: $("#countryManual").val(),
            address: $("#addressManual").val(),
            pass: $("#passManual").val(),
            checkPass: $("#checkPassManual").val()
        };

        $.ajax({
            type: "POST",
            url: "assets/api/emailInvite/manualUserAdd.php",
            //url: "email/emailInvite.php",
            data: formDataManual
        }).done(function(result) {
            //var data = jQuery.parseJSON(result);
            ...
            if (result.success == 0) {
                ...
                $("#errMsgManual").html('<i class="fa fa-times-circle"></i>' + result.message).show().delay(3000).hide("slow");
            } else if (result.success == 1) {
                $("#msgSuccessManual").html('<i class="fa fa-check-circle"></i>' + result.message).show().delay(2000).hide("slow");
            }
        });
    });
    /* Delete User */
    ...
    /* Deactivate User */
    $(document).on('click', '.deactivate_modal', function() {
        $("#deactivateModal").modal("show");
        var deactivateId = $(this).attr("id").split("_")[1];

        $(".deactivate_btn").click(function() {
            $.ajax({
                type: "POST",
                url: "backend/admin/activate.php",
                data: {
                    id: deactivateId, type:0
                },
                success: function(data) {
                    //location.reload();
                    console.log(data);
    ...
    /* Activate User */
    $(document).on('click', '.activate_modal', function() {
        $("#activateModal").modal("show");
        var activateId = $(this).attr("id").split("_")[1];
    ...
    $(document).on('click', '.reset_modal', function() {
        ...
        $(".reset_btn").click(function() {
            var formDataReset = {
                id: resetId,
                pass: $("#passReset").val(),
                checkPass: $("#checkPassReset").val()
            };
            $.ajax({
                type: "POST",
                url: "backend/admin/reset.php",
                data: formDataReset,
                success: function(data) {
                    //location.reload();
                    //console.log(data);
                    if (data.success == 0) {
                        ...
                        $("#errMsgPass").html('<i class="fa fa-times-circle"></i>' + data.message).show().delay(3000).hide("slow");
                    } else if (data.success == 1) {
                        $("#msgSuccessPass").html('<i class="fa fa-check-circle"></i>You have successfully updated the password!').show().delay(2000).hide("slow");
                    }
                },
                error: function(err) {
                    console.log(err);
